...
import { onLoginSuccess, onLoginFailure } from './features/auth/actions';
import history from './history';

const handleAuthentication = (nextState, dispatch) => {
  if (/access_token|id_token|error/.test(nextState.location.hash)) {
    const success = (token, profile) => dispatch(onLoginSuccess(token, profile));
    const failure = () => dispatch(onLoginFailure());
